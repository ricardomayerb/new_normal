...
real_gdp_lcu_hp <- add_ts_filters(real_gdp_lcu_long) %>% arrange(country, date) %>% 
  group_by(country) %>% 
  mutate(trend_growth_pct = 100*(hp_trend / dplyr::lag(hp_trend)-1) )

real_gdp_percapita_lcu_hp <- add_ts_filters(real_gdp_percapita_lcu_long) %>% arrange(country, date) %>% 
  group_by(country) %>% 
  mutate(trend_growth_pct = 100*(hp_trend / dplyr::lag(hp_trend)-1) )

plot_arg <- ggplot(data = filter(real_gdp_lcu_hp, iso == "ARG"), 
                   aes(x = date, y = value)) + geom_point() + 
  geom_line(aes(y = hp_trend), col = "blue") +
  geom_line(aes(y = cf_trend), col = "red") 
