...
growth_gdp_potencial_aravena  <- read_excel("V:/USR/RMAYER/cw/new_normal/raw_data/PIB_potencial_rm.xlsx",
sheet = "Sheet2") %>% 
...
real_gdp_percapita_lcu_long <- WEOApr2017cepal18_others_long %>% 
  filter(weo_subject_code %in% c("NGDPRPC")) %>% 
  mutate(date =  ymd(paste0(year,  "-12-31"))) 
...
real_gdp_lcu_filters_pfun <- real_gdp_lcu_filters %>% 
  ...
  left_join(growth_gdp_potencial_aravena, by = c("iso2c", "year"))

# %>% 
#   ungroup() %>% arrange(date, country) %>% group_by(date) %>% 
#   mutate(gdp_weight = value/sum(value))

gr_lac_filters_pfun <- real_gdp_lcu_filters_pfun %>% 
  filter(iso2c %in% cepal_18_countries[["iso2c"]]) %>% 
  group_by(date) %>% 
  summarise(w_gr_pfun = mean(100*growth_pot_gdp*pond),
            w_gr_hp = mean(hp_trend_growth_pct*pond),
            w_gr_cf = mean(cf_trend_growth_pct*pond),
            w_gr_actual_gdp = mean(actual_gdp_growth_pct*pond)) %>% 
  ungroup() %>% 
  filter(year(date) >= 1990 & year(date) <= 2015)

gr_lac_filters_pfun_xts <- as.xts(gr_lac_filters_pfun[,2:5], order.by = gr_lac_filters_pfun$date) 

plot_lac18_wavg_gr <- ggplot(gr_lac_filters_pfun, aes(x = date)) + 
  geom_line(aes(y = w_gr_actual_gdp, col = "actual gdp")) + 
  geom_line(aes(y = w_gr_hp, col = "HP")) + 
  geom_line(aes(y = w_gr_pfun, col = "Prod fun"))

plot_lac18_wavg_gr

plot_bra_chgs <-  ggplot(data = filter(real_gdp_lcu_filters_pfun, 
                                       iso == "BRA" & year >= 1990 & year <= 2015), 
                   aes(x = date, y = actual_gdp_growth_pct)) + geom_line() + 
...
plot_mex_chgs <-  ggplot(data = filter(real_gdp_lcu_filters_pfun, 
                                       iso == "BRA" & year >= 1990 & year <= 2015), 
                   aes(x = date, y = actual_gdp_growth_pct)) + geom_line() + 
  geom_line(aes(y = hp_trend_growth_pct), col = "blue") +
  geom_line(aes(y = cf_trend_growth_pct), col = "red") + 
  geom_line(aes(y = 100*growth_pot_gdp), col = "green") + 
  geom_smooth(aes(y = 100*growth_pot_gdp), col = "yellow", se = FALSE)
...
for (co in cepal_18_countries[["iso2c"]]) {
  ...
  co_plot <- ggplot(data = df, aes(x = date, y = actual_gdp_growth_pct)) + 
    geom_line() + 
  geom_line(aes(y = hp_trend_growth_pct), col = "blue") +
  geom_line(aes(y = 100*growth_pot_gdp), col = "green") + 
    ggtitle(co)
