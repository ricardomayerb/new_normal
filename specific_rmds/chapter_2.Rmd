...
exportdatafromplot <- function(plot_list, adir, names_vector, awbname) {
  ...
  for (i in seq(1, length(names_vector))  ) {
    ...
    write.xlsx(x = df, file = xlsxname, sheetName = datasheet)
...
for (i  in seq(1, length(figs_names))) {

  exportdatafromplot(figs_to_export[[i]], adir=figdir,
                   aname = figs_names[[i]], awbname = wbname)
...
# ggsave(file = imagename, plot = aplot)
#   # unlink(imagename)
# xlsx::addPicture(file = imagename, sheet = aname)
