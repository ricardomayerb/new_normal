...
us_gdp_deflator <- read_excel("../raw_data/us_gdp_deflator.xlsx", col_types = c("date", "numeric")) %>% 
  rename(us_deflator = value) %>% 
...
La Figura XYZ, explora la relación entre las variaciones de los términos de intercambio en los países de la región y la estructura productiva de las exportaciones en cada economía. En particular, para cada país, en base a datos de CEPAL sobre exportaciones anuales por tipo de bienes,  se calcula la participación promedio de las manufacturas en el volumen exportado --que es bastante estable en el mediano plazo pero se promedia para suavizar variaciones pequeñas, durante los años 2002 a 2008  y  por otro lado, se usa un índice de términos de intercambio con base en el nivel del año 2000, calculado por la UNCATD y obtenidos via el Banco Mudial, y con ello se computa la variación porcentual de este índice antes y después de la crisis financiera global: 2002-2008 y 2010-2015.  Es claro como la evolución de los términos de intercambio favoreció en mayor medida a los países que exportaban muy pocas manufacturas (es decir alimentos en bruto, hidrocarburo, metales, etc.) durante dicho período comparado con las economías manufactureras, para luego obtener en promedio condiciones mucho menos favorables post crisis y de hecho se invierte la relación, siendo en general más beneficiadas las economías manufactureras en el período post crisis.

```{r plot_index_x_sh_vs_tot, fig.width=8.5}
library(ggrepel)
...
plot_tot_02_08_vs_struc <- ggplot(data = x_sh_and_tot_chg_lac18,
                             aes(x = manuf_avg_00_08, y = pct_change_tot_02_08,
                                 label = iso2c)) + 
  geom_point(aes(color = subregion, shape = subregion), size = 3) +
  labs(title = "2002-2008",
       x = "exportación manufacturas (%)",
       y = "variacion términos de intercambio (%)") + 
  geom_hline(yintercept = 0) +
...
addWorksheet(newwb, sheetName = "fig_11a")
df_fig11a <- plot_tot_02_08_vs_struc$data %>% 
  select(manuf_avg_00_08, pct_change_tot_02_08, iso2c, subregion)
writeData(newwb, "fig_11a" , df_fig11a)

addWorksheet(newwb, sheetName = "fig_11b")
df_fig11b <- plot_tot_10_15_vs_struc$data %>% 
  select(manuf_avg_00_08, pct_change_tot_10_15, iso2c, subregion)
writeData(newwb, "fig_11b" , df_fig11b) 

# guides(color = guide_legend("sub-región"), shape = guide_legend("sub-región")) 

# plot_tot_10_15_vs_struc 
# plot_tot_02_08_vs_struc

easyGgplot2::ggplot2.multiplot(plot_tot_02_08_vs_struc, plot_tot_10_15_vs_struc, cols = 2)
...
```{r plot_index_x_sh_vs_nrflows, fig.width=8.5}
load("../produced_data/all_bops_lac")



nrf_avgs_lac18 <- nrf_by_country_year %>% 
  mutate(date_id =  ymd(paste0(year,  "-12-31"))) %>% 
  rename(eco_id = pais,
         voi = total_nrf) %>% 
  select(eco_id, date_id, voi) %>% 
  means_report(start1 = 2002, end1 = 2008, start2 = 2010, end2 = 2016) 


nrf_avgs_lac18_byc <- nrf_avgs_lac18$avgs_df %>% 
  mutate(iso2c = countrycode(eco_id, "iso3c", "iso2c")) %>% 
  left_join(parti_x_m_p_avg_00_08_lac18 , by = "iso2c" )


plot_nrf_00_08_vs_struc <- ggplot(data = nrf_avgs_lac18_byc,
                             aes(x = manuf_avg_00_08, y = mean_per1,
                                 label = iso2c)) +
  geom_point(aes(color = subregion, shape = subregion), size = 3) +
  labs(title = "2000-2008",
       x = "exportación manufacturas (%)",
       y = "flujo promedio anual") +
  geom_hline(yintercept = 0) +
  guides(color = FALSE, shape = FALSE) +
  geom_text_repel(size = 3) +
  theme_classic() + 
...
plot_nrf_10_16_vs_struc <- ggplot(data = nrf_avgs_lac18_byc,
                             aes(x = manuf_avg_00_08, y = mean_per2,
                                 label = iso2c)) +
  geom_point(aes(color = subregion, shape = subregion), size = 3) +
  labs(title = "2010-2016",
       x = "exportación manufacturas (%)",
       y = "flujo promedio anual") +
  geom_hline(yintercept = 0) +
  guides(color = FALSE, shape = FALSE) +
...
# easyGgplot2::ggplot2.multiplot(plot_nrf_00_08_vs_struc, plot_nrf_10_16_vs_struc, cols = 2)

```



```{r plot_index_x_sh_vs_nrflowsgdp, fig.width=8.5}

weo_gdp_usd_lac18 <- WEOApr2017cepal18_others_long %>% 
  filter(weo_subject_code == "NGDPD" & iso %in% cepal_18_countries[["iso3c"]]) %>% 
  select(weo_subject_code, iso, year, value) %>% 
  mutate(iso2c = countrycode(iso, "iso3c", "iso2c"),
         year = as.numeric(year)) %>% 
  rename(gdp_usd = value)

nrf_by_country_year_withgdp <- nrf_by_country_year %>% 
  rename(iso = pais) %>% 
  left_join(weo_gdp_usd_lac18, by = c("iso","year")) %>% 
  mutate(total_to_gdp = total_nrf/gdp_usd,
         ied_eed_to_gdp = ied_eed_y/gdp_usd,
         p_cartera_to_gdp = p_cartera_y/gdp_usd,
         p_otra_to_gdp = p_otra_y/gdp_usd)

nrf_to_gdp_avgs_lac18 <- nrf_by_country_year_withgdp %>% 
  mutate(date_id =  ymd(paste0(year,  "-12-31"))) %>% 
  rename(eco_id = iso,
         voi = total_to_gdp) %>% 
  select(eco_id, date_id, voi) %>% 
...
nrf_to_gdp_avgs_lac18_byc <- nrf_to_gdp_avgs_lac18$avgs_df %>% 
  mutate(iso2c = countrycode(eco_id, "iso3c", "iso2c")) %>% 
  left_join(parti_x_m_p_avg_00_08_lac18 , by = "iso2c" )
  

plot_nrf_to_gdp_00_08_vs_struc <- ggplot(data = nrf_to_gdp_avgs_lac18_byc,
                             aes(x = manuf_avg_00_08, y = mean_per1,
                                 label = iso2c)) +
  geom_point(aes(color = subregion, shape = subregion), size = 3) +
  labs(title = "2002-2008",
       x = "exportación manufacturas (%)",
       y = "flujo promedio anual (% PIB)") +
  geom_hline(yintercept = 0) +
  guides(color = FALSE, shape = FALSE) +
  geom_text_repel(size = 3) +
  theme_classic() + 
...
plot_nrf_to_gdp_10_16_vs_struc <- ggplot(data = nrf_to_gdp_avgs_lac18_byc,
                             aes(x = manuf_avg_00_08, y = mean_per2,
                                 label = iso2c)) +
  geom_point(aes(color = subregion, shape = subregion), size = 3) +
  labs(title = "2010-2016",
...
addWorksheet(newwb, sheetName = "fig_10a")
df_fig10a <- plot_nrf_to_gdp_00_08_vs_struc$data %>% 
  select(manuf_avg_00_08, mean_per1, iso2c, subregion)
writeData(newwb, "fig_10a" , df_fig10a)

addWorksheet(newwb, sheetName = "fig_10b")
df_fig10b <- plot_nrf_to_gdp_10_16_vs_struc$data %>% 
  select(manuf_avg_00_08, mean_per2, iso2c, subregion)
writeData(newwb, "fig_10b" , df_fig10b) 


easyGgplot2::ggplot2.multiplot(plot_nrf_to_gdp_00_08_vs_struc,
                               plot_nrf_to_gdp_10_16_vs_struc, cols = 2)
...
gdp_usd_lac18_aggr <- weo_gdp_usd_lac18 %>% 
  group_by(year) %>% 
  summarise(gdp_total_lac18 = sum(gdp_usd, rm.na = TRUE))

gdp_usd_lac17_aggr <- weo_gdp_usd_lac18 %>%
  filter(iso != "VEN" ) %>% 
  group_by(year) %>% 
  summarise(gdp_total_lac17 = sum(gdp_usd, rm.na = TRUE))
...
gross_inflows_lac18_by_type_y_usd2009 <- gross_inflows_lac18_by_type_y %>% 
  left_join(us_gdp_deflator, by = "year") %>% 
  mutate(deflated_value = 100*value/us_deflator)


gross_inflows_lac17_by_type_y_usd2009 <- gross_inflows_lac17_by_type_y %>% 
  left_join(us_gdp_deflator, by = "year") %>% 
  mutate(deflated_value = 100*value/us_deflator)
...
net_inflows_lac17_by_type_y_usd2009 <- net_inflows_lac17_by_type_y %>% 
  left_join(us_gdp_deflator, by = "year") %>% 
  mutate(deflated_value = 100*value/us_deflator)
...
yearly_net_inflows_usd2009 = ggplot(data = net_inflows_lac17_by_type_y_usd2009,
       aes(x = year, y=deflated_value, fill = inflow)) + geom_bar(stat = "identity")+ 
  ggtitle("Flujos netos por tipo de capital, LAC-17, anual")
...
yearly_gross_inflows_usd2009  <- ggplot(data = gross_inflows_lac17_by_type_y_usd2009,
       aes(x = year, y=deflated_value/1000, fill = inflow)) + geom_bar(stat = "identity") +
       scale_fill_discrete(labels = c("IED", "Otra", "Cartera")) +
  labs(title = "Brutos",
       x = "",
       y = "Miles de millones USD (2009)") +
  coord_cartesian(ylim = c(-6*10, 4*100)) +
  theme_classic() + 
...
easyGgplot2::ggplot2.multiplot(yearly_gross_inflows_usd2009,
                               yearly_net_inflows_usd2009, cols = 1)

addWorksheet(newwb, sheetName = "fig_9a")
df_fig9a <- yearly_gross_inflows_usd2009$data %>% 
  select(year, deflated_value, inflow) %>% 
  spread(year, deflated_value)

writeData(newwb, "fig_9a" , df_fig9a)

addWorksheet(newwb, sheetName = "fig_9b")
df_fig9b <- yearly_net_inflows_usd2009$data %>% 
  select(year, deflated_value, inflow) %>% 
  spread(year, deflated_value)
writeData(newwb, "fig_9b" , df_fig9b)  

saveWorkbook(newwb, "figuras_excel_9ab.xlsx", overwrite = TRUE)
...
```{r figure_credit_gap_lac_bis, message=FALSE, warning=FALSE, fig.width=8.5}

plot_credigap_lac <- ggplot(bis_credit_gdp_gap_lac %>% 
                              filter(year(date_id) > 1994),
                            aes(x = date_id, y = voi, col = eco_id)) 
plot_credigap_lac <- plot_credigap_lac + geom_line(size = 1.3) + geom_hline(yintercept = 0) + 
  labs(x = "",
       y = "Brecha de crédito (%)") +
  theme_classic() +
  guides(color=guide_legend(title=NULL)) 

addWorksheet(newwb, sheetName = "fig_12")

df_fig12 <- plot_credigap_lac$data %>% 
  select(date_id,  voi, eco_id) %>% 
  spread(eco_id, voi)

writeData(newwb, "fig_12" , df_fig12) 
...
thistable <- knitr::kable(table_data_pot_lac , digits = 1,
...
isocol <- cepal_18_countries[["iso3c"]]
expogroupcol <- c("ap", "hc", "ap", "mi", "hc", "mf", "mf", "hc", "mf", "mf",
                       "mf", "mf", "mf", "mf", "ap", "mi", "ap", "hc")
geogroupcol <- c("SA", "SA", "SA", "SA", "SA", "CARDM", "CARDM", "SA", "CARDM",
                 "CARDM", "CARDM", "CARDM", "CARDM", "CARDM" ,"SA", "SA", "SA", "SA")

lac18_expo_groups <- data.frame(iso = isocol, expogroup = expogroupcol, subregion = geogroupcol)

lac18_expo_groups <- lac18_expo_groups %>% 
  left_join(weo_lac18plus_country_dict, by = "iso")

cleve_plot_data <- lac18_expo_groups %>% 
  rename(eco_id = country) %>% 
  left_join(table_data_pot_lac, by = "eco_id") %>% 
  mutate(eco_id = factor(eco_id),
         eco_id = reorder(eco_id, change_geo))



# cleve_plot_tg_lac <- ggdotchart(cleve_plot_data,  x = "change_geo", label = "eco_id" ,
#                                 group = "expogroup", col = "expogroup")
# 
# 
# cleve_plot_tg_lac

plot_tgchange_by_expo <- ggplot(cleve_plot_data,  aes(x = reorder(eco_id, change_geo))) + 
  geom_bar(aes(fill = expogroup, y = change_geo), stat = "identity") + 
  coord_flip() +
  theme_pubr() + 
  xlab("") + ylab("% PIB tendencial") +
  scale_fill_locuszoom()
  
plot_tgchange_by_sa_cardm <- ggplot(cleve_plot_data,  aes(x = reorder(eco_id, change_geo))) + 
  geom_bar(aes(fill = subregion, y = change_geo), stat = "identity") + 
  coord_flip() +
  theme_pubr() + 
  xlab("") + ylab("% PIB tendencial") +
  scale_fill_locuszoom()

ggplot(cleve_plot_data ,  aes(x = reorder(eco_id, geo_mean_gr_per2) )) + 
  geom_bar(aes(fill = expogroup, y = geo_mean_gr_per2), stat = "identity") +
  geom_point(aes(y = geo_mean_gr_per1), shape = 18, size=3) + 
  xlab("") + ylab("% PIB tendencial") + 
  coord_flip() + 
  theme_pubr() +
  scale_fill_locuszoom()
...
ggplot(cleve_plot_data ,  aes(x = reorder(eco_id, geo_mean_gr_per2) )) + 
  geom_bar(aes(fill = subregion, y = geo_mean_gr_per2), stat = "identity") +
  geom_point(aes(y = geo_mean_gr_per1), shape = 18, size = 3) + 
  coord_flip() + 
  theme_pubr() + 
  xlab("") + ylab("% PIB tendencial")

plot_tgchange_by_expo
plot_tgchange_by_sa_cardm
...
```{r save_plots_as_named_list}

figs_to_export <- list("fig_9a" = plot_tot_02_08_vs_struc, "fig_9b" = plot_tot_10_15_vs_struc, "fig_10a" = yearly_gross_inflows_usd2009, "fig_10b" = yearly_net_inflows_usd2009, "fig_11a" = plot_nrf_to_gdp_00_08_vs_struc, "fig_11b" = plot_nrf_to_gdp_10_16_vs_struc, "fig_12" = plot_credigap_lac)



figs_names <- c("fig_9a", "fig_9b", "fig_10a", "fig_10b",
                "fig_11a", "fig_11b",  "fig_12")

save(figs_to_export, file = "list_of_final_figures")



# pathtozip = "C:\\Users\\RMAYER\\7-Zip\\7z.exe"
# pathtozip = "C://Program Files//WinRAR//WinRAR.exe"
# Sys.setenv("R_ZIPCMD" = pathtozip)

# exportdatafromplot <- function(plot_list, adir, names_vector, awbname) {
#   
#   thiswb = createWorkbook()
#   pathandfile <- paste(adir, awbname, sep = "/")
#   xlsxname <- paste0(pathandfile, ".xlsx")
#   
#   print(class(thiswb))
#   
#   for (i in seq(1, length(names_vector))  ) {
#     aname <- names_vector[[i]]
#     aplot <- plot_list[[i]]
#     
#     datasheet <- paste(aname, "data", sep = "_")
#     
#     imagename <- paste0(adir,"/", aname, ".png")
#     
#     df <- ggplot_build(aplot)$data
#     addWorksheet(wb = thiswb, sheetName = datasheet)
#    
#     writeData(wb = thiswb, datasheet, x = df)
#     
#     ggsave(file = imagename, plot = aplot)
#   }
#   saveWorkbook(wb = thiswb, xlsxname, overwrite = TRUE)
#   
# }

# figdir = "../figures_exported_data"
# wbname = "all_figs"
# 
# figs_to_export <- list(plot_tot_02_08_vs_struc, plot_tot_10_15_vs_struc,
#                        plot_credigap_lac)
# 
# figs_names <- c("fig_4a", "fig_4b", "fig_5")
# adir = "../figures_exported_data"
# awbname = wbname
# names_vector = figs_names
# plot_list = figs_to_export
# 
# thiswb = openxlsx::createWorkbook()
# pathandfile <- paste(adir, awbname, sep = "/")
# xlsxname <- paste0(pathandfile, ".xlsx")
# print(class(thiswb))
# for (i in seq(1, length(names_vector))  ) {
...
#   imagename <- paste0(adir,"/", aname, ".png")
#   df <- ggplot_build(aplot)$data
#   openxlsx::addWorksheet(wb = thiswb, sheetName = datasheet)
#   openxlsx::writeData(wb = thiswb, datasheet, x = df)
#   ggsave(file = imagename, plot = aplot)
# }
# 
# openxlsx::saveWorkbook(wb = thiswb, "foo.xlsx", overwrite = TRUE)
# openXL(file = "foo.xlsx")
# 

# exportdatafromplot(plot_list = figs_to_export, adir=figdir,
#                    names_vector = figs_names, awbname = wbname)
# 

```

```{r saveworkbook}
saveWorkbook(newwb, "figuras_excel.xlsx", overwrite = TRUE)
```
