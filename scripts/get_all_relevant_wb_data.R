...
gfcf_to_coLCU <- gfcf_to_coLCU %>% 
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  mutate(gfcf_gr = simple_gross_growth(value)) %>% 
  ungroup()
  
gcf_to_coLCU <- gcf_to_coLCU %>% 
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  mutate(gcf_gr = simple_gross_growth(value)) %>% 
  ungroup()
...
gfcf_to_coLCU_03_08 <- gfcf_to_coLCU %>% 
  ...
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  summarise(gfcf_gagr_03_08 = geomean_growth(value),
...
gfcf_to_coLCU_10_15 <- gfcf_to_coLCU %>% 
  ...
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  summarise(gfcf_gagr_10_15 = geomean_growth(value),
...
gcf_to_coLCU_03_08 <- gcf_to_coLCU %>% 
  ...
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  summarise(gcf_gagr_03_08 = geomean_growth(value),
...
gcf_to_coLCU_10_15 <- gfcf_to_coLCU %>% 
  ...
  arrange(iso2c, date) %>% 
  group_by(iso2c) %>% 
  summarise(gcf_gagr_10_15 = geomean_growth(value),
